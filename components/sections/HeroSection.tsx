...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-brand-bg-primary pt-20">
      {/* Ambient glow blobs */}
      <div className="pointer-events-none absolute left-0 top-0 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/4 rounded-full bg-brand-accent-gold/5 blur-[120px]" />
      ...
      >
        <Link
          ...
          className="flex items-center gap-2 rounded-full border border-brand-border bg-brand-text-primary/5 px-5 py-2.5 font-body text-xs font-medium text-brand-text-primary/80 backdrop-blur-md transition-colors hover:border-brand-accent-gold/50 hover:text-brand-accent-gold"
        >
      ...
      <div className="relative mx-auto flex min-h-[calc(100svh-5rem-120px)] max-w-7xl flex-col items-start px-6 sm:px-8 lg:flex-row lg:items-center lg:px-12">
        ...
        <div className="relative z-10 flex flex-1 flex-col items-center justify-center pt-12 text-center lg:items-start lg:pt-0 lg:text-left">
          <motion.h1
            {...entry(0.1)}
            className="max-w-2xl font-heading text-5xl font-bold leading-[1.1] tracking-tight text-brand-text-primary sm:text-6xl md:text-[64px] lg:text-[72px]"
          >
          ...
          <motion.p
            {...entry(0.24)}
            className="mt-5 max-w-lg font-body text-sm leading-relaxed text-brand-text-secondary sm:text-base md:text-lg lg:max-w-md"
          >
            {HERO_COPY.subtext}
          </motion.p>
          ...
          <motion.div {...entry(0.36)} className="mt-8 flex justify-center lg:justify-start">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-full bg-brand-accent-gold px-7 py-3.5 font-body text-sm font-semibold text-brand-text-primary shadow-lg shadow-brand-accent-gold/30 transition-all hover:bg-[#976d29] hover:shadow-brand-accent-gold/50 active:scale-95"
            >
              {HERO_COPY.cta.label}
              <ArrowRight size={15} />
            </Link>
          ...
          <motion.div
            {...entry(0.48)}
            className="mx-auto mt-10 flex w-fit items-center gap-4 rounded-2xl border border-brand-border bg-brand-bg-secondary px-4 py-3 backdrop-blur-sm lg:mx-0"
          >
            <div className="flex items-center">
              {CUSTOMER_AVATARS.map((a, i) => (
                <div
                  key={i}
                  className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-brand-bg-primary font-body text-xs font-bold text-brand-text-primary"
            ...
            <div>
              <p className="font-body text-[11px] font-semibold text-brand-text-primary">Our Happy Customers</p>
              <div className="flex items-center gap-1">
                <Star size={10} className="fill-brand-accent-gold text-brand-accent-gold" />
                <span className="font-body text-[10px] text-brand-text-secondary">4.9 · 2,400+ reviews</span>
              </div>
            </div>
          </motion.div>
        </div>
